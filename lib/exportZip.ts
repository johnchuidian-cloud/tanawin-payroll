...
// One payslip, one file — for re-issues and single copies. No zip, no
// storage side effects, constant memory at any staff count.
export async function buildSinglePayslip(opts: {
  format: ExportFormat;
  slip: Payslip;
  periodStart: SheetDate | null;
  periodEnd: SheetDate | null;
  disbursement: SheetDate | null;
  node?: HTMLElement | null; // PNG mode captures this rendered preview
}): Promise<ZipResult> {
  const { format, slip, periodStart, periodEnd, disbursement, node } = opts;
  const base = payslipFilename(slip.family, slip.given, disbursement).replace(/\.pdf$/, '');
  if (format === 'pdf') {
    const { payslipPdfBlob } = await import('./pdf');
    return { blob: await payslipPdfBlob(slip, periodStart, periodEnd), filename: `${base}.pdf` };
  }
...
export async function buildPayslipZip(opts: {
  ...
  // Pairing list so Maya can copy-paste addresses without opening the workbook.
