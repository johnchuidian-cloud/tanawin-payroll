...
export const RIDE_PRICE = 100; // ride package 1, per pax
export const RIDE2_PRICE = 150; // ride package 2, per pax (added 2026-08-27)
export const FORT_PRICE = 50; // Fort of Fun, per pax (added 2026-08-27)
...
export interface Loan {
  ...
  note: string | null;
  active: boolean;
  // First payday this loan deducts on (ISO), so a loan can be entered the
  // day it's granted — before amortization begins — instead of remembered
  // later (santos's Aug 10 red: loan granted in July, forgotten until payday).
  startsOn?: string | null;
}

// One labeled amount — "uniform share — 600" instead of a bare 600, so the
// archive can explain itself months later without a sheet screenshot.
...
export interface EmployeeInputs {
  ...
  // Labeled versions of adjustments/others. When present (non-empty), they
  // REPLACE the bare numbers above — never both. The bare fields survive for
  // old drafts and the tmp/ harnesses.
  adjustItems?: LineItem[];
  deductItems?: LineItem[];
  note?: string; // free text saved onto this person's payslip record
}
...
export interface CutoffInputs {
  ...
  restDays: number;
  ridePax: number | ''; // ride package 1
  ride2Pax: number | ''; // ride package 2
  fortPax: number | ''; // Fort of Fun
...
// Labeled items as stored on a payslip record: blank rows dropped, amounts
// resolved to numbers. Null when nothing meaningful was entered.
function cleanItems(items?: LineItem[]): { label: string; amount: number }[] | null {
  const kept = (items ?? [])
    .filter((it) => it.label.trim() !== '' || n(it.amount) !== 0)
    .map((it) => ({ label: it.label.trim(), amount: n(it.amount) }));
  return kept.length ? kept : null;
}

const iso3 = (d: SheetDate): string =>
...
export function tipsPool(inputs: CutoffInputs): number {
  return (
    n(inputs.ridePax) * RIDE_PRICE +
    n(inputs.ride2Pax) * RIDE2_PRICE +
    n(inputs.fortPax) * FORT_PRICE +
    n(inputs.serviceCharge) +
    n(inputs.horsePax) * HORSE_PRICE +
    n(inputs.tipBox)
  );
}
...
export function computeRun(
  ...
  if (pool > 0) {
    warnings.push({
      severity: 'info',
      message: `Tips pool ₱${pool.toFixed(2)} (ride pkg 1 ${n(inputs.ridePax)}×${RIDE_PRICE} + ride pkg 2 ${n(inputs.ride2Pax)}×${RIDE2_PRICE} + fort of fun ${n(inputs.fortPax)}×${FORT_PRICE} + service ${n(inputs.serviceCharge).toFixed(2)} + horse ${n(inputs.horsePax)}×${HORSE_PRICE} + tip box ${n(inputs.tipBox).toFixed(2)}) ÷ ${headcount} = ${tipShare.toFixed(2)} each.`,
    });
  }
  ...
  for (const e of included) {
    ...
    const adjTotal = inp.adjustItems?.length
      ? inp.adjustItems.reduce((a, it) => a + n(it.amount), 0)
      : n(inp.adjustments);
    const eAdjustments = adjTotal !== 0 ? adjTotal : null;
    ...
    const disbIso = iso3(inputs.disbursementDate);
    const myLoans = loans.filter(
      (l) => l.employeeId === e.id && l.active && (!l.startsOn || l.startsOn <= disbIso)
    );
    ...
    const deductTotal = inp.deductItems?.length
      ? inp.deductItems.reduce((a, it) => a + n(it.amount), 0)
      : n(inp.others);
    const dOthers = deductTotal !== 0 ? deductTotal : null;
    ...
    payslips.push({
      ...
      adjustItems: cleanItems(inp.adjustItems),
      deductItems: cleanItems(inp.deductItems),
    });
...
export function compareRuns(
  ...
  const s = byFam(sheet);
  // A rename mid-parallel (reyes → reyes-cruz) shows up as one person
