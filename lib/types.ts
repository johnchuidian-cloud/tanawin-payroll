...
export interface Payslip {
  ...
  netPay: number;
  // Cutoff notes (2026-09-25): internal context saved with the record —
  // NEVER rendered on the payslip an employee receives.
  note?: string | null; // free text about this person this cutoff
  adjustItems?: { label: string; amount: number }[] | null; // labeled additions
  deductItems?: { label: string; amount: number }[] | null; // labeled deductions
}
