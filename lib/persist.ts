...
export interface RunSummary {
  ...
  source: RunSource;
  created_at: string;
  note?: string | null; // present once migration-007 has run
}
...
export interface LoadedRun {
  ...
  payslips: Payslip[];
  warnings: RunWarning[];
  note: string | null; // the run-level note (null pre-migration-007)
}
...
// True once migration-007 (cutoff notes: runs.note, payslips.note/line_items)
// has been applied. Saves must not name the new columns before then.
let notesReadyCache: boolean | null = null;
export async function notesReady(): Promise<boolean> {
  if (!supabase) return false;
  if (notesReadyCache !== null) return notesReadyCache;
  const { error } = await supabase.from('payroll_runs').select('note').limit(1);
  notesReadyCache = !error;
  return notesReadyCache;
}

// Edit the run-level note of an already-archived run (late context is normal:
// "found out later the 600 was uniform share" belongs on the record).
export async function updateRunNote(runId: string, note: string): Promise<void> {
  if (!supabase) throw new Error('Archive storage is not connected.');
  if (!(await notesReady())) {
    throw new Error('Notes need a one-time database update — run migration-007-cutoff-notes.sql in Supabase.');
  }
...
export async function listRuns(): Promise<RunSummary[]> {
  ...
  const { data, error } = await supabase
    ...
    // ~10 years of paired sheet+computed runs. Explicit, because PostgREST
    // would otherwise cap silently at 1000; newest-first means anything past
    // the limit is decade-old history (still in the DB, just off the landing).
    .limit(500);
...
export async function saveComputedRun(
  ...
  // Notes/labels need migration-007. Pre-migration, the run still saves (the
  // amounts are correct either way) — but losing the labels must be LOUD.
  const withNotes = await notesReady();
  const hasNotes =
    !!runNote?.trim() ||
    payslips.some((s) => s.note || s.adjustItems?.length || s.deductItems?.length);
  if (!withNotes && hasNotes) {
    warnings = [
      ...warnings,
      {
        severity: 'warning',
        message:
          'Notes and labels were NOT stored — run migration-007-cutoff-notes.sql in Supabase, then compute and save this cutoff again.',
      },
    ];
  }
  ...
  const { data: runRow, error: runErr } = await supabase
    ...
    .insert({
      ...
      source: 'computed',
      ...(withNotes ? { note: runNote?.trim() || null } : {}),
    })
    .select('id')
    .single();
  ...
  const rows = payslips.map((s) => ({
    ...
    ...(withNotes
      ? {
          note: s.note ?? null,
          line_items:
            s.adjustItems?.length || s.deductItems?.length
              ? { adds: s.adjustItems ?? [], deducts: s.deductItems ?? [] }
              : null,
        }
      : {}),
  }));
...
export async function loadRun(id: string): Promise<LoadedRun> {
  ...
  const payslips: Payslip[] = (slips ?? []).map((r) => ({
    ...
    netPay: r.net_pay ?? 0,
    note: r.note ?? null,
    adjustItems: (r.line_items as { adds?: { label: string; amount: number }[] } | null)?.adds ?? null,
    deductItems: (r.line_items as { deducts?: { label: string; amount: number }[] } | null)?.deducts ?? null,
  }));
