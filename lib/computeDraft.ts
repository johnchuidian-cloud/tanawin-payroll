...
// bug cost Maya her Aug 10 entries). Session-scoped; a successful Save to
...
export interface ComputeDraft {
  ...
  restDays: number | '';
  ridePax: number | '';
  // Older saved drafts predate these two — readers must treat missing as ''.
  ride2Pax?: number | '';
  fortPax?: number | '';
  serviceCharge: number | '';
  horsePax: number | '';
  tipBox: number | '';
  // Run-level note, saved onto the archived run. Older drafts predate it.
  runNote?: string;
  perEmp: Record<string, EmployeeInputs>;
}

// The Compute form edits labeled items only; older drafts (and the sheet
// import) carry bare adjustments/others numbers. Convert once on read: the
// bare amount becomes a single unlabeled row and the bare field is cleared,
// so the engine can never see both and double-count.
export function toItemInputs(inp: EmployeeInputs): EmployeeInputs {
  const out: EmployeeInputs = {
    ...inp,
    adjustItems: inp.adjustItems ?? [],
    deductItems: inp.deductItems ?? [],
    note: inp.note ?? '',
  };
  if (!out.adjustItems!.length && inp.adjustments !== '' && inp.adjustments != null) {
    out.adjustItems = [{ label: '', amount: inp.adjustments }];
  }
  if (!out.deductItems!.length && inp.others !== '' && inp.others != null) {
    out.deductItems = [{ label: '', amount: inp.others }];
  }
  out.adjustments = '';
  out.others = '';
  return out;
