...
// only loads when Maya actually clicks download.
import { Document, Font, Page, StyleSheet, Text, View, pdf } from '@react-pdf/renderer';

// A payslip line must never break mid-word ("accu-rate").
Font.registerHyphenationCallback((word) => [word]);
import { COMPANY_NAME, COMPANY_NAME_SIGNATURE, COMPANY_SIGNATORY } from './company';
...
export function PayslipDoc({
  ...
  return (
    <Document title={`Payslip — ${slip.family}`} author={COMPANY_NAME}>
      <Page size="A4" style={styles.page}>
        ...
        <View style={[styles.cols, { marginTop: 32 }]}>
          <View style={styles.col}>
            ...
            <Text style={{ marginTop: 56 }}>{COMPANY_SIGNATORY}</Text>
            <Text>{COMPANY_NAME_SIGNATURE}</Text>
          </View>
          <View style={styles.col}>
