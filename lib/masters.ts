...
export async function loadMasters(
 ...
 if (!supabase) throw new Error('Storage is not connected.');
  ...
  const [emp, lns] = await Promise.all([
    empQuery,
    // Newest first + a hard limit: PostgREST silently caps un-limited selects
    // at 1000 rows, and ended loans accumulate forever. Descending order makes
    // any truncation drop ANCIENT ended loans — never a current deduction.
    // (Ascending order here would eventually make active loans vanish and the
    // payroll silently under-deduct.)
