...
// (e.g. "navvaro" and "navaro" are DIFFERENT people one letter apart).
...
export function looksSimilar(a: string, b: string): boolean {
  // hyphen/space-boundary prefix: "reyes" vs "reyes-cruz"
...
export function buildRun(wb: ParsedWorkbook): BuildResult {
  ...
  for (const p of wb.payrollRows) {
    ...
    // totals the columns can't explain (dizon's June final pay was typed
