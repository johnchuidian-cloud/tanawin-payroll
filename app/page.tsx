...
import type { ParsedWorkbook, Payslip, RunWarning } from '@/lib/types';
...
interface ResultState {
  ...
  runNote?: string | null;
  runId: string | null;
  // Present only for a fresh sheet upload (not archived views): powers the
  // "match the app to this sheet" panel.
  wb?: ParsedWorkbook;
}
...
export default function Home() {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      if (supabase && session) {
        setResult({ meta, payslips: run.payslips, warnings: run.warnings, savedNote: 'Archiving…', runId: null, wb });
        try {
    ...
    <main className="min-h-screen">
      <header className="bg-[#9A3518] text-[#FBF6EF] px-5 py-4 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm opacity-80">Payroll · {COMPANY_NAME}</p>
        </div>
      ...
      <div className="max-w-5xl mx-auto p-5 space-y-5">
        ...
        {signedIn && session !== undefined && result && (
          <ResultView
            ...
            runNote={result.runNote ?? null}
            runId={result.runId}
            belowActions={result.wb && supabase && session ? <SheetSyncPanel wb={result.wb} /> : undefined}
            onDeleted={() => {
              setResult(null);
              listRuns().then(setRuns).catch(() => {});
            }}
                        ...
                        {g.runs.map((r) => (
                          <li key={r.id}>
                            ...
                            >
                              ...
                              {r.note && (
                                <span className="text-sm text-[#4A3526] block truncate">📝 {r.note}</span>
                              )}
                            </button>
                          </li>
                        ))}
