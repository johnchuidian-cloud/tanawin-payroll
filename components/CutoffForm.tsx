...
import { clearDraft, cutoffDates, readDraft, writeDraft, type ComputeDraft } from '@/lib/computeDraft';
...
export default function CutoffForm({
      ...
      .then((m) => {
        ...
        setPerEmp(
          Object.fromEntries(
            m.employees.map((e) => [
              e.id,
              { ...EMPTY_EMP, include: e.employeeType !== 'daily', ...(draft?.perEmp?.[e.id] ?? {}) },
            ])
          )
        );
      })
  ...
  useEffect(() => {
    const d: ComputeDraft = { periodStart, periodEnd, disbursement, restDays, ridePax, serviceCharge, horsePax, tipBox, perEmp };
    writeDraft(d);
  }, [periodStart, periodEnd, disbursement, restDays, ridePax, serviceCharge, horsePax, tipBox, perEmp]);
  ...
  }, [masters, dates, restDays, ridePax, serviceCharge, horsePax, tipBox, perEmp]);
