...
// page (Maya's instinct on Aug 10 — she tried to compute reyes-cruz from
  ...
  const result = useMemo(() => {
    ...
    try {
      const run = computeRun(employees, loans, {
        ...
        ride2Pax: computeDraft?.ride2Pax ?? '',
        fortPax: computeDraft?.fortPax ?? '',
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Payout">
            <select className={INPUT} value={d.payoutChannel} onChange={(e) => set({ payoutChannel: e.target.value as Draft['payoutChannel'] })}>
              ...
              <option value="manual">📱 GCash/BPI (Tala)</option>
            </select>
          </Field>
          <div />
