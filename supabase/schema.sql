...
create table payroll_runs (
  ...
  note              text,            -- migration-007: one note about the whole cutoff
...
create table payslips (
  ...
  -- migration-007: internal context — never rendered on the payslip itself
  note             text,               -- free text about this person this cutoff
  line_items       jsonb,              -- {"adds":[{label,amount}],"deducts":[...]}
...
-- Staff-only, single-user (Maya). Default deny: RLS on, anon gets nothing.
-- The only access path is an authenticated Supabase Auth user (Maya's login,
